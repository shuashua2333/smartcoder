...
const axios_1 = __importDefault(require("axios")); // 引入网络库
const cheerio = __importStar(require("cheerio")); // 引入 HTML 解析库
const path = __importStar(require("path")); // 引入路径处理库
// === 差异视图内容提供者 ===
...
class DiagnosticTreeItem extends vscode.TreeItem {
    ...
    documentUri;
    lineNumber;
    constructor(diagnostic, documentUri, lineNumber) {
        super(`第 ${lineNumber + 1} 行: ${diagnostic.message}`, vscode.TreeItemCollapsibleState.None);
...
class DiagnosticsTreeDataProvider {
    ...
    constructor(_sidebarProvider) {
        this._sidebarProvider = _sidebarProvider;
        // 监听诊断变化
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("smartCoderView", sidebarProvider));
    // 2. 注册诊断树视图
    const diagnosticsTreeProvider = new DiagnosticsTreeDataProvider(sidebarProvider);
    context.subscriptions.push(vscode.window.createTreeView('smartcoder-diagnostics', {
        treeDataProvider: diagnosticsTreeProvider,
        showCollapseAll: false
    }));
    // 3. 注册 AI 修复诊断命令
    context.subscriptions.push(vscode.commands.registerCommand('smartcoder.fixDiagnostic', async (item) => {
        await sidebarProvider.fixDiagnostic(item.diagnostic, item.documentUri, item.lineNumber);
    }));
    // 4. 注册快捷键命令 (Alt+A)
    context.subscriptions.push(vscode.commands.registerCommand('smartcoder.start', () => {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
    ...
    // 5. 注册终端崩溃分析命令
    context.subscriptions.push(vscode.commands.registerCommand('smartcoder.analyzeTerminal', async () => {
        await sidebarProvider.analyzeRuntimeError();
    }));
    // 6. 注册生成单元测试命令
    context.subscriptions.push(vscode.commands.registerCommand('smartcoder.generateUnitTest', async () => {
        await sidebarProvider.generateUnitTest();
    }));
}
class SmartCoderSidebarProvider {
    _extensionUri;
    _diffProvider;
    _view;
    _history = [];
    constructor(_extensionUri, _diffProvider) {
        this._extensionUri = _extensionUri;
        this._diffProvider = _diffProvider;
    }
        ...
        webviewView.webview.onDidReceiveMessage(async (data) => {
            switch (data.type) {
                ...
                case 'askAI':
                    ...
                    break;
                case 'loadProblem': // 🔥 新增：加载题目
                    this._handleLoadProblem(data.value);
                    break;
            }
        });
    }
    handleUserSelection(code) {
        ...
        try {
            ...
            if (looksLikeError && previousClipboard.trim().length > 20) {
                ...
                if (!errorLog.trim() || errorLog === previousClipboard) {
                    // 提示用户手动复制错误信息
                    const action = await vscode.window.showWarningMessage('未检测到错误信息。\n\n使用方法：\n1. 在终端中选中报错信息（会自动复制）\n2. 按 Ctrl+Shift+E 或通过命令面板运行"SmartCoder: 分析运行时错误"', '打开命令面板', '知道了');
                    if (action === '打开命令面板') {
...
- 参数：${functionInfo.parameters.length > 0 ? functionInfo.parameters.map((p) => `${p.type} ${p.name}`).join(', ') : '无'}
- 所在类：${className || '未知'}
- 命名空间：${namespace || '未知'}
...
⚠️ 必须返回 JSON 格式：{ "analysis": "Markdown格式的测试说明（包含测试覆盖的场景说明）", "code": "完整的测试类代码（包含所有必要的 using 语句和命名空间）" }`;
    ...
    // === AI 生成单元测试专用调用 ===
    async _callAiForUnitTest(prompt, functionInfo, document, functionRange) {
        if (!this._view)
            return;
        try {
...
不要使用 markdown 代码块包裹 JSON。`;
    ...
    // === 解析函数信息 ===
    _parseFunctionInfo(code, document, range) {
    ...
    // === 解析参数列表 ===
    _parseParameters(parametersStr) {
    ...
    // === 检测光标位置的函数 ===
    _detectFunctionAtPosition(document, position) {
    ...
    // === 🔥 新增功能：AI 修复诊断错误 ===
    async fixDiagnostic(diagnostic, documentUri, lineNumber) {
        if (!this._view) {
            // 如果侧边栏未打开，先打开它
            await vscode.commands.executeCommand('smartCoderView.focus');
            await new Promise(resolve => setTimeout(resolve, 300));
...
⚠️ 必须返回 JSON 格式：{ "analysis": "Markdown格式的分析文本", "code": "修复后的代码片段（只包含需要修改的部分，保持原有缩进）" }`;
    ...
    async _callAiForDiagnosticFix(prompt, diagnostic, documentUri, lineNumber, errorLineCode, contextCode) {
        if (!this._view)
            return;
        try {
            const config = vscode.workspace.getConfiguration('smartcoder');
...
不要使用 markdown 代码块包裹 JSON。`;
    ...
    // === 🔥 新增功能：处理题目加载 ===
    async _handleLoadProblem(input) {
        if (!this._view)
            return;
        // 1. 通知前端显示加载状态
    ...
    // === 通用 AI 调用 (JSON 模式) ===
    async _callAiWithHistory(userMessage, codeContext = "") {
        if (!this._view)
            return;
...
不要使用 markdown 代码块包裹 JSON。`;
    ...
    // === 🔥 智能代码应用：支持智能覆盖和诊断修复 ===
    async _applyCodeToEditor(code, diagnosticFix, unitTest) {
        // 情况0：单元测试模式（创建新测试文件）
        if (unitTest) {
            try {
                ...
                if (fileExists) {
                    // 文件已存在，询问用户
                    const action = await vscode.window.showWarningMessage(`测试文件 ${testFileName} 已存在，是否覆盖？`, '覆盖', '追加', '取消');
                    if (action === '取消') {
                        return;
                    }
                    if (action === '覆盖') {
        ...
        if (diagnosticFix) {
            try {
                ...
                const range = new vscode.Range(new vscode.Position(diagnosticFix.range.start.line, diagnosticFix.range.start.character), new vscode.Position(diagnosticFix.range.end.line, diagnosticFix.range.end.character));
                // 打开目标文档并应用修复
        ...
        switch (codeType.type) {
            ...
            case 'full_file':
                // 完整文件，替换整个文档
                targetRange = new vscode.Range(document.positionAt(0), document.positionAt(fullText.length));
                break;
        }
        if (targetRange) {
            // 显示预览并确认
            const preview = document.getText(targetRange);
            const action = await vscode.window.showInformationMessage(`检测到 ${codeType.type === 'class' ? '类' : codeType.type === 'method' ? '方法' : '完整文件'}，是否替换？`, '替换', '取消', '查看差异');
            if (action === '替换') {
                await editor.edit(builder => {
            ...
            // 无法智能匹配，提供选项
            const action = await vscode.window.showInformationMessage('无法自动匹配代码位置，请选择操作：', '插入到光标位置', '替换整个文件', '取消');
            if (action === '插入到光标位置') {
                await editor.edit(builder => {
                ...
                const confirm = await vscode.window.showWarningMessage('确定要替换整个文件吗？此操作不可撤销。', '确定', '取消');
                if (confirm === '确定') {
                    await editor.edit(builder => {
    ...
    // 查找类定义的范围
    _findClassRange(document, className) {
        const text = document.getText();
    ...
    // 查找方法定义的范围
    _findMethodRange(document, methodName) {
        const text = document.getText();
    ...
    // 保留缩进辅助方法
    _preserveIndentation(code, baseIndent) {
    ...
    // 显示差异视图
    async _showDiff(oldCode, newCode, label) {
        try {
            // 生成唯一的 URI（使用时间戳避免冲突）
            ...
            // 打开差异视图
            await vscode.commands.executeCommand('vscode.diff', oldUri, newUri, `${label} (原代码) ↔ ${label} (新代码)`);
        }
        catch (error) {
